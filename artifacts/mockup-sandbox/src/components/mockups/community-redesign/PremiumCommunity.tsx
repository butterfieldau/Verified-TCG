...
const posts = [
  {
    id: "mara-charizard", initials: "MC", name: "Mara Chen", handle: "@marachens", time: "18 min", color: "#B04A53",
  ...
  {
    id: "jonah-gyarados", initials: "JL", name: "Jonah Lewis", handle: "@jonahpulls", time: "1 hr", color: "#3E7182",
...
export function PremiumCommunity() {
  ...
  const [liked, setLiked] = useState<Record<string, boolean>>(() => Object.fromEntries(posts.map((post) => [post.id, post.liked])));
  const [saved, setSaved] = useState<string[]>([]);
  const [following, setFollowing] = useState<Record<string, boolean>>(() => Object.fromEntries(collectors.map((collector) => [collector.handle, collector.following])));
  ...
  const [commentSent, setCommentSent] = useState<Record<string, boolean>>({});
  const [conversation, setConversation] = useState<typeof messages[number] | null>(null);
  ...
  const publishPost = () => {
    ...
    const id = `rafael-${Date.now()}`;
    setFeedPosts((current) => [{
      id, initials: "RV", name: "Rafael Velez", handle: "@rafaelcollects", time: "now", color: "#B04A53",
      copy: draft.trim(), card: photo ? "New find · Attached photo" : "Collector note", set: photo || "Shared with your community",
      art: "charizard", likes: 0, comments: 0, liked: false, detail: photo ? "Photo attached" : "Verified collector note",
    }, ...current]);
    ...
    setComposer(false);
    setTab("feed");
    announce("Post published to your community");
  };
  ...
  return (
    <main className="community-shell min-h-[100dvh] bg-[#0A0A0A] text-white selection:bg-[#FF1E2D]/30">
        ...
        <section className="min-w-0 flex-1 lg:max-w-[760px] lg:border-r lg:border-[#2A2A2A]">
          ...
          {tab === "feed" && <div className="space-y-4 px-4 pb-10 sm:px-6 lg:px-10">
            <button onClick={() => setComposer(true)} className="group flex min-h-[70px] w-full items-center gap-3 rounded-2xl border border-dashed border-[#3A3A3A] bg-[#141414] p-3 text-left transition hover:border-[#FF1E2D]/70"><Avatar initials="RV" color="#B04A53" /><div className="flex-1"><p className="text-sm font-medium text-white">What are you displaying today?</p><p className="mt-1 text-[11px] text-[#9A9A9A]">Share a card, a story, or a sharp eye.</p></div><div className="flex h-10 w-10 items-center justify-center rounded-xl bg-[#2A2A2A] text-[#FF1E2D]"><Camera size={18} /></div></button>
             {feedPosts.map((post) => <article key={post.id} className="overflow-hidden rounded-2xl border border-[#2A2A2A] bg-[#1A1A1A] shadow-[0_8px_30px_rgba(0,0,0,.18)]">
              ...
              <div className="mx-4 mb-3 flex overflow-hidden rounded-xl border border-[#2A2A2A] bg-[#141414]"><div className="w-[39%]"><CardArt kind={post.art} /></div><div className="flex flex-1 flex-col justify-between p-3"><div><p className="text-[9px] font-semibold uppercase tracking-[.14em] text-[#9A9A9A]">Card reference</p><h3 className="mt-2 text-[16px] font-bold leading-5 tracking-[-.02em]">{post.card}</h3><p className="mt-1 text-[10px] text-[#9A9A9A]">{post.set}</p></div><div className="flex items-center gap-1.5 text-[10px] text-[#D4AF37]"><Star size={12} fill="currentColor" /> {post.detail}</div></div></div>
               <div className="flex items-center border-t border-[#2A2A2A] px-3 py-1"><IconButton label={liked[post.id] ? "Unlike post" : "Like post"} active={liked[post.id]} onClick={() => setLiked((v) => ({ ...v, [post.id]: !v[post.id] }))}><Heart size={17} fill={liked[post.id] ? "currentColor" : "none"} /></IconButton><span className="-ml-1 mr-2 text-[11px] text-[#9A9A9A]">{post.likes + (liked[post.id] && !post.liked ? 1 : 0)}</span><IconButton label="Comment on post" onClick={() => { setCommenting(post.id); setCommentDraft(""); }}><MessageCircle size={17} /></IconButton><span className="-ml-1 text-[11px] text-[#9A9A9A]">{post.comments + (commentSent[post.id] ? 1 : 0)}</span><div className="ml-auto flex"><IconButton label="Save post" active={saved.includes(post.id)} onClick={() => setSaved((v) => v.includes(post.id) ? v.filter((x) => x !== post.id) : [...v, post.id])}><Bookmark size={17} fill={saved.includes(post.id) ? "currentColor" : "none"} /></IconButton><IconButton label="Share post" onClick={() => announce("Post link copied")}><Share2 size={17} /></IconButton></div></div>
            </article>)}
          </div>}

           {tab === "discover" && <div className="space-y-6 px-4 pb-12 sm:px-6 lg:px-10"><div className="flex items-center gap-3 rounded-xl border border-[#2A2A2A] bg-[#141414] px-4 py-2"><Search size={17} className="text-[#9A9A9A]" /><input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search names, sets, specialties" className="min-h-10 flex-1 bg-transparent text-sm outline-none placeholder:text-[#666]" />{query && <button aria-label="Clear search" onClick={() => setQuery("")} className="text-[#9A9A9A]"><X size={16} /></button>}</div><div className="rounded-2xl border border-[#2A2A2A] bg-[#141414] p-5"><div className="flex items-center gap-2 text-[#FF1E2D]"><Sparkles size={16} /><span className="text-[9px] font-bold uppercase tracking-[.18em]">Curated for you</span></div><h2 className="mt-2 text-2xl font-bold tracking-[-.04em]">Collectors with a sharp eye</h2><p className="mt-1 text-xs text-[#9A9A9A]">Based on your Base Set shelf and recent saves.</p></div>{results.length > 0 ? results.map((c) => <div key={c.handle} className="flex items-center gap-3 border-b border-[#2A2A2A] pb-4"><Avatar initials={c.initials} color={c.color} /><div className="min-w-0 flex-1"><p className="text-sm font-semibold">{c.name}</p><p className="text-[11px] text-[#9A9A9A]">{c.handle} · {c.note}</p></div><button onClick={() => { setFollowing((v) => ({ ...v, [c.handle]: !v[c.handle] })); announce(following[c.handle] ? `Unfollowed ${c.name}` : `Following ${c.name}`); }} className={`min-h-10 rounded-lg border px-3 text-[11px] font-semibold transition ${following[c.handle] ? "border-[#3A3A3A] text-[#9A9A9A]" : "border-[#FF1E2D] text-[#FF1E2D] hover:bg-[#FF1E2D]/10"}`}>{following[c.handle] ? "Following" : "Follow"}</button></div>) : <div className="rounded-xl border border-dashed border-[#3A3A3A] p-6 text-center text-xs text-[#9A9A9A]">No collectors match that search yet.</div>}</div>}
           {tab === "messages" && <div className="space-y-2 px-4 pb-12 sm:px-6 lg:px-10">{messages.map((m, i) => <button key={m[1]} onClick={() => setConversation(m)} className="flex min-h-[82px] w-full items-center gap-3 rounded-xl p-3 text-left transition hover:bg-[#141414]"><Avatar initials={m[0]} color={m[4]} /><div className="min-w-0 flex-1"><div className="flex justify-between"><span className="text-sm font-semibold">{m[1]}</span><span className="text-[10px] text-[#9A9A9A]">{m[3]}</span></div><p className="mt-1 truncate text-xs text-[#9A9A9A]">{m[2]}</p></div>{i === 0 && <span className="h-2 w-2 rounded-full bg-[#FF1E2D]" />}</button>)}</div>}
        </section>
      ...
      {notice && <div role="status" className="fixed bottom-5 left-1/2 z-30 -translate-x-1/2 rounded-full border border-[#3A3A3A] bg-[#1A1A1A] px-4 py-2 text-xs font-medium text-white shadow-xl">{notice}</div>}
       {composer && <div className="fixed inset-0 z-20 flex items-end justify-center bg-black/70 p-0 sm:items-center sm:p-6"><div className="w-full max-w-[520px] rounded-t-2xl border border-[#2A2A2A] bg-[#1A1A1A] p-5 shadow-2xl sm:rounded-2xl"><div className="flex items-center justify-between"><div><p className="text-[9px] font-bold uppercase tracking-[.18em] text-[#9A9A9A]">New community post</p><h2 className="mt-1 text-2xl font-bold tracking-[-.04em]">Share a find</h2></div><IconButton label="Close composer" onClick={() => setComposer(false)}><X size={20} /></IconButton></div><div className="mt-5 flex gap-3"><Avatar initials="RV" color="#B04A53" /><textarea autoFocus value={draft} onChange={(e) => setDraft(e.target.value)} placeholder="Share a card, a story, or a sharp eye..." className="min-h-[110px] flex-1 resize-none bg-transparent text-sm leading-6 outline-none placeholder:text-[#666]" /></div>{photo && <p className="mt-2 pl-[52px] text-xs text-[#22C55E]">Photo attached: {photo}</p>}<div className="mt-4 flex items-center justify-between border-t border-[#2A2A2A] pt-4"><input ref={fileRef} type="file" accept="image/*" className="hidden" onChange={(e) => setPhoto(e.target.files?.[0]?.name ?? "")} /><button onClick={() => fileRef.current?.click()} className="flex min-h-11 items-center gap-2 rounded-lg px-3 text-xs text-[#9A9A9A] hover:bg-white/5"><ImageIcon size={17} /> Add photo</button><button disabled={!draft.trim()} onClick={publishPost} className="flex min-h-11 items-center gap-2 rounded-lg bg-[#FF1E2D] px-5 text-xs font-bold text-white transition hover:bg-[#e51b29] disabled:opacity-40">Publish <Send size={14} /></button></div></div></div>}
       {commenting && <div className="fixed inset-0 z-20 flex items-end justify-center bg-black/70 p-0 sm:items-center sm:p-6"><div className="w-full max-w-[520px] rounded-t-2xl border border-[#2A2A2A] bg-[#1A1A1A] p-5 shadow-2xl sm:rounded-2xl"><div className="flex items-center justify-between"><div><p className="text-[9px] font-bold uppercase tracking-[.18em] text-[#9A9A9A]">Conversation</p><h2 className="mt-1 text-2xl font-bold tracking-[-.04em]">Add a thoughtful note</h2></div><IconButton label="Close comments" onClick={() => setCommenting(null)}><X size={20} /></IconButton></div><textarea autoFocus value={commentDraft} onChange={(e) => setCommentDraft(e.target.value)} placeholder="Say something useful..." className="mt-5 min-h-[100px] w-full resize-none rounded-xl border border-[#2A2A2A] bg-[#141414] p-3 text-sm leading-6 outline-none placeholder:text-[#666]" /><button disabled={!commentDraft.trim()} onClick={() => { setCommentSent((current) => ({ ...current, [commenting]: true })); setCommenting(null); setCommentDraft(""); announce("Comment added"); }} className="mt-4 flex min-h-11 w-full items-center justify-center gap-2 rounded-xl bg-[#FF1E2D] text-xs font-bold text-white transition hover:bg-[#e51b29] disabled:opacity-40">Add comment <Send size={14} /></button></div></div>}
       {conversation && <div className="fixed inset-0 z-20 flex items-end justify-center bg-black/70 p-0 sm:items-center sm:p-6"><div className="w-full max-w-[520px] rounded-t-2xl border border-[#2A2A2A] bg-[#1A1A1A] p-5 shadow-2xl sm:rounded-2xl"><div className="flex items-center justify-between"><div className="flex items-center gap-3"><Avatar initials={conversation[0]} color={conversation[4]} /><div><p className="text-sm font-semibold">{conversation[1]}</p><p className="text-[10px] text-[#22C55E]">Verified collector</p></div></div><IconButton label="Close conversation" onClick={() => setConversation(null)}><X size={20} /></IconButton></div><p className="mt-5 rounded-xl border border-[#2A2A2A] bg-[#141414] p-4 text-sm leading-6 text-[#D0D0D0]">{conversation[2]}</p><button onClick={() => { setConversation(null); announce(`Replying to ${conversation[1]}`); }} className="mt-4 flex min-h-11 w-full items-center justify-center gap-2 rounded-xl border border-[#FF1E2D] text-xs font-bold text-[#FF6A73]">Reply <Send size={14} /></button></div></div>}
    </main>
  );
}