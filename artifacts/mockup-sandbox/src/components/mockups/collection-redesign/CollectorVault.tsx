...
type Panel = "settings" | "filters" | "lists" | "sort" | null;
type Card = {
  id: number;
  name: string;
  set: string;
...
export function CollectorVault() {
  ...
  useEffect(() => {
    ...
    const dismissSearchOnOutsidePress = (event: PointerEvent) => {
      if (searchAreaRef.current && !searchAreaRef.current.contains(event.target as Node)) {
        setSearchOpen(false);
      }
    };
    document.addEventListener("pointerdown", dismissSearchOnOutsidePress);
    return () => document.removeEventListener("pointerdown", dismissSearchOnOutsidePress);
  }, [searchOpen]);
  ...
  return <main className="vtcg-collection min-h-[100dvh] overflow-hidden pb-24">
    <div className="mx-auto max-w-[430px]">
      <section className="px-5 pb-4 pt-5 text-center">
      ...
      {summary === "performance" && <section className="performance-chart-enter mx-5 mb-5 rounded-2xl border border-[#2b4637] bg-[#14231b] p-4">
       ...
       <section className="px-4">
        <div className="mb-3 flex items-end justify-between"><div><p className="text-[10px] font-bold uppercase tracking-[1.6px] text-[#8d888b]">THE VAULT</p><h2 className="tcg-display text-[25px] font-bold">YOUR CARDS <span className="font-sans text-sm font-normal text-[#8d888b]">· {results.length}</span></h2></div><div className="flex items-center gap-2"><div className="flex rounded-md border border-[#303035] bg-[#151517] p-0.5"><button aria-label="Grid view" onClick={() => setView("grid")} className={`grid h-8 w-8 place-items-center rounded ${view === "grid" ? "bg-[#373039] text-white" : "text-[#777278]"}`}><Grid2X2 size={14} /></button><button aria-label="List view" onClick={() => setView("list")} className={`grid h-8 w-8 place-items-center rounded ${view === "list" ? "bg-[#373039] text-white" : "text-[#777278]"}`}><List size={14} /></button></div><RoundButton label="Add card" onClick={() => flash("Card intake ready")}><Plus size={18} /></RoundButton></div></div>
          <div ref={searchAreaRef} className="mb-3 flex items-center gap-2">
            <div className={`flex min-w-0 flex-1 items-center gap-3 rounded-full border bg-[#151517] px-4 py-2.5 transition-colors ${searchOpen ? "border-[#a24953]" : "border-[#353338]"}`}>
           ...
           <RoundButton label="Open collection settings" onClick={() => setPanel("settings")}><SlidersHorizontal size={17} /></RoundButton>
         </div>
    ...
    {selecting && <div className="fixed bottom-[76px] left-1/2 z-30 flex w-[calc(100%-32px)] max-w-[398px] -translate-x-1/2 items-center gap-1 rounded-2xl border border-[#4c393d] bg-[#211b1e]/[.98] p-2 shadow-2xl"><button onClick={() => { setSelecting(false); setSelected([]); }} className="px-2 text-xs text-[#9a9295]">Done</button><span className="mr-auto text-xs font-bold">{selected.length} selected</span><button onClick={() => flash("Marked selected cards for sale")} className="rounded-lg bg-[#ff1e2d] px-2.5 py-2 text-[10px] font-bold">Sale</button><button onClick={() => flash("Trade list updated")} className="rounded-lg bg-[#d6a85e]/15 px-2.5 py-2 text-[10px] font-bold text-[#e8c986]">Trade</button><button aria-label="Delete selected cards" onClick={() => { flash("Removal requires confirmation in the full app"); setSelected([]); }} className="rounded-lg bg-[#ff5967]/10 p-2 text-[#ff5967]"><Trash2 size={14} /></button></div>}
     {panel && <div className="settings-backdrop fixed inset-0 z-40 overflow-y-auto bg-black/70" onClick={() => setPanel(null)}><div onClick={event => event.stopPropagation()} className="settings-sheet fixed bottom-0 left-0 right-0 mx-auto w-full max-w-[430px] rounded-t-[24px] border-t border-[#4a363b] bg-[#19191c] p-5"><div className="mb-5 flex items-center justify-between"><h2 className="tcg-display text-2xl font-bold">{panel === "settings" ? "COLLECTION SETTINGS" : panel === "filters" ? "FILTER THE VAULT" : panel === "lists" ? "PORTFOLIO LISTS" : "SORT LIBRARY"}</h2><button aria-label="Close panel" onClick={() => setPanel(null)}><X size={18} /></button></div>{panel === "settings" && <div className="space-y-2.5"><button onClick={() => setPanel("filters")} className="settings-panel-row flex w-full items-center gap-3 rounded-[15px] border border-[#303035] bg-[#151517] px-4 py-3 text-left"><Filter size={17} className="text-[#ff5967]" /><span className="flex-1"><b className="block text-[13px]">Filters</b><small className="text-[11px] text-[#8d888b]">{filter === "All" ? "All cards" : filter}</small></span><ChevronRight size={16} className="text-[#777278]" /></button><button onClick={() => setPanel("lists")} className="settings-panel-row flex w-full items-center gap-3 rounded-[15px] border border-[#303035] bg-[#151517] px-4 py-3 text-left"><Folder size={17} className="text-[#d6a85e]" /><span className="flex-1"><b className="block text-[13px]">Portfolio list</b><small className="text-[11px] text-[#8d888b]">{activeList}</small></span><ChevronRight size={16} className="text-[#777278]" /></button><button onClick={() => setPanel("sort")} className="settings-panel-row flex w-full items-center gap-3 rounded-[15px] border border-[#303035] bg-[#151517] px-4 py-3 text-left"><ArrowDown size={17} className="text-[#d6a85e]" /><span className="flex-1"><b className="block text-[13px]">Sort library</b><small className="text-[11px] text-[#8d888b]">{sort} · {ascending ? "ascending" : "descending"}</small></span><ChevronRight size={16} className="text-[#777278]" /></button><div className="settings-panel-row flex items-center justify-between rounded-[15px] border border-[#303035] bg-[#151517] px-4 py-3"><span><b className="block text-[13px]">View</b><small className="text-[11px] text-[#8d888b]">Choose grid or list</small></span><div className="flex rounded-md border border-[#303035] bg-[#19191c] p-0.5"><button aria-label="Grid view" onClick={() => setView("grid")} className={`grid h-9 w-9 items-center justify-center rounded ${view === "grid" ? "bg-[#373039] text-white" : "text-[#777278]"}`}><Grid2X2 size={15} /></button><button aria-label="List view" onClick={() => setView("list")} className={`grid h-9 w-9 items-center justify-center rounded ${view === "list" ? "bg-[#373039] text-white" : "text-[#777278]"}`}><List size={15} /></button></div></div></div>}{panel === "filters" && <><p className="mb-3 text-xs text-[#8d888b]">Choose one slice of the vault. Tap outside this sheet to close it.</p><div className="grid grid-cols-3 gap-2">{filters.map(item => <button key={item} onClick={() => setPanel("filters")} className={`rounded-xl border p-3 text-left text-[11px] font-bold ${filter === item ? "border-[#71323a] bg-[#311d22] text-[#ff9ca4]" : "border-[#303035] bg-[#151517] text-[#bcb4b4]"}`}>{item}<small className="mt-1 block text-[9px] font-normal text-[#8d888b]">{item === "All" ? `${cards.length} cards` : `${cards.filter(card => item === "Pokémon" ? true : item === "Graded" ? !!card.grade : item === "Raw" ? !card.grade : item === "For Sale" ? !!card.sale : !!card.trade).length} cards`}</small></button>)}</div><div className="mt-4 grid grid-cols-2 gap-2">{["Price range", "Set / series", "Condition", "Grading company"].map(item => <button key={item} onClick={() => flash(`${item} filter selected`)} className="flex items-center justify-between rounded-xl border border-[#303035] p-3 text-left text-xs">{item}<ChevronDown size={14} className="text-[#777278]" /></button>)}</div><button onClick={() => setPanel(null)} className="mt-5 w-full rounded-xl bg-[#ff1e2d] py-3 text-xs font-bold">Apply filters</button></>}{panel === "lists" && <><div className="space-y-2">{lists.map((list, index) => <button key={list} onClick={() => { setActiveList(list); setPanel(null); }} className={`flex w-full items-center gap-3 rounded-xl border p-3 text-left ${activeList === list ? "border-[#71323a] bg-[#311d22]" : "border-[#303035]"}`}><Folder size={17} className="text-[#ff5967]" /><span className="flex-1"><b className="block text-xs">{list}</b><small className="text-[10px] text-[#8d888b]">{index === 0 ? "6 cards · AUD 1,944.75" : index === 1 ? "3 cards · AUD 1,534.50" : "2 cards · AUD 506.25"}</small></span>{activeList === list && <Check size={15} className="text-[#ff5967]" />}<MoreHorizontal size={16} className="text-[#777278]" /></button>)}</div><button onClick={() => { setLists(current => [...current, `New List ${current.length}`]); flash("New portfolio list created"); }} className="mt-4 flex w-full items-center justify-center gap-2 rounded-xl border border-dashed border-[#71323a] py-3 text-xs font-bold text-[#ff9ca4]"><Plus size={15} /> Create portfolio list</button></>}{panel === "sort" && <div className="space-y-1">{["Value", "Name", "Recent", "Quantity", "Gain"].map(item => <button key={item} onClick={() => { setSort(item); setPanel(null); }} className={`flex w-full items-center justify-between rounded-xl px-3 py-3 text-sm ${sort === item ? "bg-[#311d22] text-[#ff9ca4]" : ""}`}>{item}{sort === item && <Check size={15} />}</button>)}<button onClick={() => setAscending(current => !current)} className="mt-2 flex w-full items-center gap-2 border-t border-[#303035] pt-4 text-xs text-[#bcb4b4]">{ascending ? <ArrowUp size={14} /> : <ArrowDown size={14} />}{ascending ? "Ascending" : "Descending"}</button></div>}</div></div>}
